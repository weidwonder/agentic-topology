...
> 通用画图工具是**表达工具**——你已经知道系统长什么样，要讲给别人听。
> 这个项目是**勘测工具**——你不知道系统长什么样，要先搞清楚。
> 前者的产物是结论的载体，后者的产物是调查的起点。

所以**它只陈述读到的事实，不评价架构好坏、不给改进建议**。用途是让你看清楚到能自己下判断。

---

## 这三张图分别回答什么问题

下面三张都是真的——从一个内控审计子模块（1500 行 TypeScript）读出来的：10 个方块、14 条连线。

| 谁在干活、谁调谁 | 这一个方块具体收了什么、交出了什么 | 方块多了会不会漏 |
|---|---|---|
| [![编排全貌](./assets/images/overview.png)](./assets/images/overview.png) | [![节点详情](./assets/images/detail.png)](./assets/images/detail.png) | [![折叠视图](./assets/images/folded.png)](./assets/images/folded.png) |
| 底色只说这个方块是谁在干：蓝的是 AI、橙的是程序、紫的是岔路口。点一份信息的名字，传它的那几条线一起亮起来。 | 点开任何一个方块，弹层给出它的提示词范围、能用的工具、收发的信息——**每一条都钉着 `文件:行号`**。 | 按堆收起来，一个方块一条线都不会少；展开回去，逐个 ID 都对得上。 |

> 图上的正文——方块名、干什么、每份信息是什么——都是被分析项目自己的话，`--lang en` 也不会翻译它。所以英文那份 README 配的是一份英文样例的图，见 [English README](./README.md)。
...
**你不需要写任何描述文件、不需要敲任何其它命令。** 不放心背后发生了什么、要不要自己写？见后面
[《那个 `<描述文件>` 是什么？要我写吗？》](#那个-描述文件-是什么要我写吗)一节。

---

## 它守着哪些边界

跟通用画图工具比，这才是真正的差异——不是免责声明，是写进测试、改了就红的约束：
...
**还差**：上线线要求**在 3 个真实项目上 agent 节点与调用边遗漏 = 0**，目前**未达标**。
2026-09-21 把泄题面程序化排查了一遍，结果比原先记的更差——**跑过的三个基准项目里有两个永久出局**：

1. **两个项目泄题，作废。** 根因是结构性的：这个技能的教材和它的考题用了同一批项目。
   `multica` 被 `节点粒度约定.md`〈一个真实例子〉写着；`aiudit_platform` 更彻底，
   `节点粒度约定.md` §事实基准**逐字给出了节点数、边数、Agent 数和两处合并判断**，
   另外三份文档还各自拿它举例——而 SKILL.md 强制要求分析前把这几份都读一遍。
   也就是说按正常流程走的 agent**必然**先读到答案，盲测在设计上就不可能盲。
   （`multica` 另外还有一笔：划范围时把装配系统提示词的那个包排除在外了，
   而 Agent 的判据恰恰是"看系统提示词"。）现在只剩 `cpah-docs` 一个合规，**得另找两个**。
2. **粒度留账没有过**——"调用边遗漏 = 0"这个口径靠"合并进节点内部的传递 MUST 留账"
   兜底，那是它唯一的防作弊位。没有留账，把切法调粗、让传递沉进节点里就能刷出 0。
   所以已有的那几个 0 只是「测出来是 0」，**不是「认定为 0」**。
3. 从没有任何一轮同时满足：范围正确 + 留账齐全 + 未泄题。

这三笔欠在哪、什么才算证据、怎么防泄题，都写在 `benchmarks/README.md` 里，
`benchmarks/benchmark.mjs` 会拒绝为泄题的项目出具收据。
