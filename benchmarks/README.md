...
> **注意**：本仓的 `docs/` 整个在 `.gitignore` 里，所以下文几处指向 `docs/report/` 的引用
> **只在原作者本机可达**，克隆这个仓库的人打不开。它们是那三轮实测的原始证据来源，保留引用
> 是为了说清结论从哪来；**但凡本套件运行时真正要守的规矩，都已经写在这份文档里，不靠那些引用**。
...
这条闸直接对应下面〈选基准项目的规矩〉那句话：**划范围 MUST 覆盖系统提示词的装配处**——第三个基准项目当初整轮作废，原因之一就是把 `execenv/` 排除在范围外，而那正是装配系统提示词的地方。这条闸把"别重蹈覆辙"从人工提醒变成机械检查。
...
这是这套东西存在的主要理由之一。分三层：**选项目时就得守的规矩**、**机制上怎么防**、**这次复核实际查出了什么**。

### 选基准项目的规矩（2026-08-28 立）

- **基准项目 MUST NOT 是本技能任何一份强制加载文档拿来举例的项目。**
  举例是为了教方法；同一个项目又被拿去当考题，考的就是记性不是方法。
- **划范围 MUST 覆盖系统提示词的装配处。** Agent 的判据是"装配了一套系统提示词"，
  把装配它的那个包排除在范围外，判据就没法执行了。
- 这两条 MUST 在派发分析之前核一遍，**不是事后补救**。

### 教材项目池（2026-09-21 立）

第一条规矩立于 2026-08-28，但当时**只排查了 `multica`，没有回溯排查已经跑过的另外两个**。
2026-09-21 建这套 harness 时程序化地 grep 了一遍，才发现 `aiudit_platform` 也违反同一条规矩，
而且违反得更彻底（取证见下面〈已知泄题面〉）。规矩本身没错，**错在它从来没有被回溯执行过**。

根因是结构性的：**教材必须有具体例子，具体例子必然泄题**。所以这两个池子 MUST 互斥，
并且哪些项目属于教材池 MUST 在这里显式登记——不登记就只能靠每次现查，而现查过一次就漏过一次。

**教材项目池（这些项目 MUST NOT 再作为基准项目，永久）**：

| 项目 | 被哪些强制加载文档拿来举例 |
|---|---|
| `multica` | `references/节点粒度约定.md`〈一个真实例子〉 |
| `aiudit_platform` | 见下面〈已知泄题面〉那张表，四份文档都有 |

**"永久"的含义**：把这几份文档改到不再举某个项目的例子，**并不能让它重新具备盲测资格**。
改文档只消除这一次的泄题面，而它作为教材项目的身份不变——下次给教材加新例子时还会再泄。
所以这张表只进不出。

**新增教材例子时 MUST 同步登记到上表**，并确认那个项目不在基准池里。
`cases/*.case.json` 的 `blind_test_eligible` 字段是这条规矩的程序化落点：
标 `false` 的 case，`benchmark.mjs verify` 直接拒绝出具收据。
...
`SKILL.md` 的加载点清单要求：填字段前必读 `编排描述格式.md`、判粒度前必读 `节点粒度约定.md`、读源码前必读 `抽取纪律.md`、想看样例就读 `example.topology.yaml`——**这四份文档覆盖了正常分析流程几乎全部会读到的参考材料**，而它们都带着这个项目的痕迹。按上面〈选基准项目的规矩〉"MUST NOT 是本技能任何一份强制加载文档拿来举例的项目"这条，`aiudit-internal-control` 的状态与 `multica` 相同：**永久失去盲测资格**。

**这几份文档不需要改**——2026-09-21 使用者拍板：承认 `aiudit_platform` 是教材项目，让它退出基准池，而不是改写四份文档的例子。教材本来就该有具体例子，改写只消除这一次的泄题面，下次加新例子还会再泄。所以这个 case 的 `blind_test_eligible` **不会再翻回 `true`**，它已登记进上面的〈教材项目池〉。
...
**`third-project-pending`：待定**。选定后 MUST 先做同样的 grep 复核（见上面〈选基准项目的规矩〉），确认零命中再补数据；`multica` 与 `aiudit_platform` 都已因这条规矩**永久**失去资格，已登记进〈教材项目池〉，不能再选。
...
1. **第三个基准项目还没选定**（`third-project-pending`）。选定后需要：确认满足〈选基准项目的规矩〉两条前提（不是强制加载文档举过例的项目、划范围覆盖系统提示词装配处）；对它做一次和 `aiudit`/`cpah-docs` 同样的复核 grep；读源码补齐 `required_agent_nodes`/`required_edges`/`required_scope_refs`。
